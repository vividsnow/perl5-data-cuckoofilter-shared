...
/* True iff the whole mapped region is zero.  Recovery is a cold path,
   so a byte scan is fine. */
    ...
    if (anonymous) {
        ...
        if (!is_new) {
            if (!cf_validate_header((CfHeader *)base, (uint64_t)st.st_size)) {
                 ...
                 * ftruncate and cf_init_header below leaves a full-size, all-zero
 ...
 * The bucket table + geometry are immutable in a sealed file, so contains /
 * count_of / count read directly with no reader-slot / rwlock traffic -- the
 * mapping is never written, so it works from a read-only fd / read-only
 * filesystem and can be shared PROT_READ across processes (same architecture;
